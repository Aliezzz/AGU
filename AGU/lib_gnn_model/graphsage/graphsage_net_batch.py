...
class SAGENet(torch.nn.Module):
    def __init__(self, in_channels, h_dim, bias, self_loop, out_channels, dropout, num_layers=2):
        ...
        self.convs.append(SAGEConv(in_channels, h_dim, bias=bias, root_weight=self_loop))
        self.convs.append(SAGEConv(h_dim, out_channels, bias=bias, root_weight=self_loop))

    def forward(self, x, edge_index):
        for i in range(self.num_layers - 1):
            x = F.relu(self.convs[i](x, edge_index))
            x1 = F.dropout(x, training=self.training, p=self.dropout)
