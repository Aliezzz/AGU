...
class NodeClassifier(GNNBase):
    ...
    def determine_model(self, num_feats, num_classes):
        if self.target_model == 'SAGE':
            return SAGENet(num_feats, self.h_dim, self.bias, self.self_loop, num_classes, self.dropout)
        elif self.target_model == 'GAT':
            return GATNet(num_feats, self.gat_dim, self.num_heads, self.bias, self.self_loop, num_classes, self.dropout)
        elif self.target_model == 'GCN':
            return GCNNet(num_feats, self.h_dim, self.bias, self.self_loop, num_classes, self.dropout)
        elif self.target_model == 'GIN':
            return GINNet(num_feats, self.h_dim, self.bias, num_classes, self.dropout)
        elif self.target_model == 'SGC':
            return SGCNet(num_feats, num_classes, self.bias, self.self_loop)
        else:
    ...
    def train_model(self, unlearn_info=None):
        ...
        for epoch in range(self.args['num_epochs']):
            ...
            for batch in self.train_loader:
                ...
                optimizer.zero_grad()
                _, out = self.model(batch.x, batch.edge_index)
        ...
        _, out1 = self.model(self.data.x, self.data.edge_index)
        _, out2 = self.model(self.data.x_unlearn, self.data.edge_index_unlearn)
    ...
    def evaluate_unlearn_F1(self, new_parameters):
        ...
        for p in self.model.parameters():
            ...
            idx = idx + 1
        self.model.eval()
        _, out = self.model(self.data.x_unlearn, self.data.edge_index_unlearn)
